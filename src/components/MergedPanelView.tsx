...
export function MergedPanelView({ scopedPanelIds }: MergedPanelViewProps) {
  ...
  const panels = useMemo(() => {
    if (!scopedPanelIds) return allPanels;
    const scoped = new Map<
      string,
      typeof allPanels extends Map<string, infer V> ? V : never
    >();
    for (const id of scopedPanelIds) {
      const p = allPanels.get(id);
      if (p) scoped.set(id, p);
    }
    return scoped;
  }, [allPanels, scopedPanelIds]);
  // When scoped to a group, compute merged logs locally.
  // When unscoped (global), use the store's merged state.
  const globalMergedLogRefs = useWorkspaceStore((s) => s.mergedLogRefs);
  const globalMergedEventKeyMap = useWorkspaceStore((s) => s.mergedEventKeyMap);
  ...
  // Compute merged logs (locally when scoped, globally when not)
  ...
  // For global mode, recompute when panel logs change
  const panelLogHash = useMemo(() => {
    if (scopedPanelIds) return ""; // local mode handles its own recompute
    const parts: string[] = [];
    for (const [panelId, panel] of panels) {
      if (panel.logGroupName) {
  ...
  useEffect(() => {
    if (!scopedPanelIds) {
      recomputeGlobalMergedLogs();
    }
  }, [panelLogHash, scopedPanelIds, recomputeGlobalMergedLogs]);
